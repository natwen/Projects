...
def un_cheval(ma_ligne : int, keep_running, positions) : # ma_ligne commence à 0
    col=1
...
def arbitre(positions):
    ...
    cheval_premier = 0
    cheval_dernier = 0
    while position[cheval_dernier] <LONGEUR_COURSE :
        position = positions[:].copy()
        for i in range(1,len(position)):
            if position[i] > position[cheval_premier]:
                cheval_premier = i
            elif position[i] < position[cheval_dernier]:
                cheval_dernier = i 

        move_to(22,1)
        en_couleur(lyst_colors[0])
        print("le meilleur cheval est :", chr(ord('A')+cheval_premier))
        move_to(23,1)
        en_couleur(lyst_colors[0])
        print("le moins bon cheval est : ", chr(ord('A')+cheval_dernier))
#------------------------------------------------
...
def course_hippique(keep_running) :
    ...
    for i in range(Nb_process):  # Lancer     Nb_process  processus
        mes_process[i] = mp.Process(target=un_cheval, args= (i,keep_running,positions,))
        mes_process[i].start()
...
if __name__ == "__main__" :
    ...
    Nb_process=20
    mes_process = [0 for i in range(Nb_process)]
    positions = mp.Array("i",[0 for i in range(20)])
    print(positions[:])
